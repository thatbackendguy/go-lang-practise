...
func main() {
	...
	*p = *p / 2 //dividing x through the pointer
	...
	fmt.Printf("Using dereferencing twice for pointer of pointer (**ppaa): %#v\n", **ppaa)

	bb := 1100

	pbb := &bb

	fmt.Println(paa == pbb) // false, even if the values of the pointer are equal

	otherPointer := &aa

	fmt.Println(otherPointer == paa) // true
}
